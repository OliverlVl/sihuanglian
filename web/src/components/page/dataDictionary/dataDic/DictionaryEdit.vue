<template>
  <el-dialog title="数据字典修改" :visible.sync="dialogVisible" :close-on-click-modal="false">
    <el-form
      ref="dictEditForm"
      :model="dictData"
      size="small"
      ...
      <el-row :gutter="20">
        <el-col :md="12" :sm="24">
          <el-form-item label="参数名称:" prop="item_key">
            <el-input v-model="dictData.item_key" placeholder="参数名称" :disabled="true"></el-input>
          </el-form-item>
        </el-col>
        <el-col :md="12" :sm="24">
...
export default {
  ...
  methods: {
    ...
    openEdit(dictData) {
      this.loadingStart();
      dictionaryMainAPI
        .selectDictionaryDetailById(dictData.detail_id)
        .then(res => {
          this.loadingEnd();
        // console.log(this.dictData)
          this.dictData.detail_id = res.data.detail_id;
          this.dictData.item_key = res.data.item_key;
          this.dictData.item_value = res.data.item_value;
        })
        .catch(err => {
          this.loadingEnd();
        //   console.error(err);
